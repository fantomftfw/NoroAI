...
export async function GET(request: NextRequest) {
  try {
    ...
    if (period === 'weekly') {
      ...
      const year = parseInt(searchParams.get('year') || now.getFullYear().toString());
      const month = parseInt(searchParams.get('month') || (now.getMonth() + 1).toString()) - 1;
    ...
    // Query for created tasks with isVoiceDump true
    ...
    // Query for completed tasks with isVoiceDump true
    ...
    console.error('Error fetching voice dump stats:', error);
    return NextResponse.json(
      { message: 'Error fetching voice dump stats' },
      { status: 500 }
    );
  }
}
